...
@dataclass
class FullMultiModalImageSelectorConfig:
    """Configuration for full multi-modal image selector.
...
class FullMultiModalImageSelector(Component):
    config: FullMultiModalImageSelectorConfig
    ...
    def __call__(
        ...
        **kwargs: object,
        ...
        """
        Select personalized images for ALL articles using multi-modal CLIP embeddings.

        Combines visual user preferences with text-image relevance for better personalization.
        Unlike selective versions, this personalizes 100% of articles.
        """
        ...
        logger.debug(
            f"Generated multi-modal user embedding, personalizing images for ALL "
        ...
        # Select best image for ALL articles (100% personalization)
        personalized_count = 0
        for article in recommendations.articles:
            if not article.images:
                continue

            # Multi-modal image selection for every article
        ...
        logger.debug(f"Personalized images for {personalized_count} out of {len(recommendations.articles)} articles")
        return recommendations
    ...
    def _select_best_image_multimodal(
        ...
        Option 2: Visual user preferences + Text-image relevance
        - visual_scores: How well each image matches user's visual taste
        - text_relevance: How well each image relates to current article's text
        """

        # Get visual similarity scores (user's visual preferences from click history)
        ...
        if text_relevance_scores is not None:
            ...
            logger.debug(f"Using multi-modal fusion (visual preference: {self.config.visual_weight}, "
                        f"text-image relevance: {self.config.text_weight})")
        else:
            # Fallback to visual-only
            final_scores = visual_scores
            logger.debug("Using visual-only selection (no article text embedding available)")
    ...
    def _compute_text_image_relevance(self, article, embedding_lookup: dict):
        """Compute how well each image relates to the CURRENT article's text content.

        This is Option 2: We compare each image to the current article's text,
        NOT to the user's text click history.
        """

        # Get CURRENT article's text embedding (not user's text history!)
        ...
        # For each image, compute cross-modal similarity with CURRENT article text
        relevance_scores = []
        for img in article.images:
            if img.image_id in embedding_lookup and "image" in embedding_lookup[img.image_id]:
                ...
                # Cross-modal similarity: how well does this image represent THIS article's text?
