...
from poprox_recommender.components.filters.image_selector_clip_multimodal_full import (
    FullMultiModalImageSelector,
    FullMultiModalImageSelectorConfig,
)
...
def configure(builder: PipelineBuilder, num_slots: int, device: str):
    ...
    # Pre-computed CLIP embeddings from V3 API (includes both image and text embeddings)
    ...
    # Full multi-modal image personalization for ALL articles (100% personalization)
    # This combines visual user preferences with text-image relevance for every article
    # No selective personalization - all articles get personalized images
    full_multimodal_config = FullMultiModalImageSelectorConfig(
        visual_weight=0.7,
        text_weight=0.3,
        max_history_visual=50
    )
    builder.add_component(
        "full_multimodal_image_personalizer",
        FullMultiModalImageSelector,
        full_multimodal_config,
        recommendations=ranker,
        interacted_articles=i_clicked,
        embedding_lookup=embedding_lookup_table,
    )

    # Expose fully personalized results as the recommender output
    builder.alias("recommender", "full_multimodal_image_personalizer")
